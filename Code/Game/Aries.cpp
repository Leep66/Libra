...
Aries::Aries(EntityType type, EntityFaction faction)
	:Entity(type, faction)
{
	...
	m_ariesBaseTexture = g_theRenderer->CreateOrGetTextureFromFile("Data/Images/EnemyAries.png");
}
...
void Aries::Update(float deltaSeconds)
{
	...
	else
	{
		...
		static const float TIME_THRESHOLD = 3.0f;
		if (m_lastTile == currentTile)
		{
			m_timeAtCurrentTarget += deltaSeconds;
			if (m_timeAtCurrentTarget >= TIME_THRESHOLD)
			{
				ChooseNewTargetPosition();
			}
		}
...
void Aries::Render() const
{
	...
	g_theRenderer->BindTexture(m_ariesBaseTexture);
...
bool Aries::IsShieldWork(RaycastResult2D result)
{
	if (!result.m_didImpact)
		return false;

	Vec2 shieldCenter = m_position;
	float shieldAngleStart = m_orientationDegrees - 45.f;
	float shieldAngleEnd = m_orientationDegrees + 45.f;

	if (shieldAngleStart < 0.f) shieldAngleStart += 360.f;
	if (shieldAngleEnd >= 360.f) shieldAngleEnd -= 360.f;

	Vec2 impactDirection = result.m_impactPos - shieldCenter;
	float impactAngle = impactDirection.GetOrientationDegrees();

	if (impactAngle < 0.f) impactAngle += 360.f;

	if (shieldAngleStart < shieldAngleEnd) {
		if (impactAngle >= shieldAngleStart && impactAngle <= shieldAngleEnd) {
			return true;
		}
	}
	else {
...
void Aries::UpdateOrientation(float deltaSeconds)
{
...
void Aries::MoveTowardTarget(float deltaSeconds)
{
...
void Aries::ChooseNewTargetPosition()
{
	Vec2 newTarget;
...
void Aries::FollowPath(float deltaSeconds)
{
	UNUSED(deltaSeconds);
